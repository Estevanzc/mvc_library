        ...
        <section id="screen_middle" class="w-full flex justify-center items-center flex-col">
            <main class="mt-10 w-[60%] flex justify-center items-center flex-col">
                <form action="salvarRetirada.php" method="POST" class="w-5/12 h-[400px] flex justify-center items-center flex-col bg-white rounded-lg shadow-2xl drop-shadow-2xl border-2 border-solid border-gray-300">
                    ...
                    $cur_date = date("Y-m-d");
                    $inc_date = increase_date();
                    ?>
                    <div class="w-full h-1/4 flex justify-center items-center flex-col gap-y-1">
                        <input type="date" name="retirada" class="hidden" value="<?php echo $cur_date?>">
                        <input type="date" name="devolucao" class="hidden" value="<?php echo $inc_date?>">
                        <div class="w-full flex justify-start items-end pl-5 gap-x-1 text-sm">
                            <p class="text-cyan-700 font-bold">Retirada:</p>
                            <p class="font-medium text-xs"><?php echo $cur_date?></p>
                        </div>
                        <div class="w-full flex justify-start items-end pl-5 gap-x-1 text-sm">
                            <p class="text-cyan-700 font-bold">Devolução:</p>
                            <p class="font-medium text-xs"><?php echo $inc_date?></p>
                        </div>
                    </div>
